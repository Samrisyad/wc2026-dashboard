...
import bracket_render
import champion_render
...
def match_card(m, extra_class=""):
    ...
    <div class="{card_class}">
      <div class="match-top">
        <span class="group-tag">{f"Group {m['group']}" if m.get('group') else m.get('stage', '')}</span>
        {status_badge(m)}
      </div>
...
for m in sorted(matches, key=lambda m: m["date_wib"]):
    g = m.get("group")
    stage = m.get("stage", "")
    group_label = f"Group {g}" if g else stage.replace("_", " ").title() if stage else ""
    home_label = with_flag_code(m['home']) if m.get('home') else "<em>TBD</em>"
    away_label = with_flag_code(m['away'], reverse=True) if m.get('away') else "<em>TBD</em>"
    schedule_rows += f"""<tr>
        <td>{fmt_date(m['date_wib'])}</td><td>{group_label}</td>
        <td class="match-cell-td"><div class="match-cell"><span class="match-team home">{home_label}</span><span class="match-vs">vs</span><span class="match-team away">{away_label}</span></div></td><td>{fmt_score(m)}</td><td>{m.get('venue','')}</td>
...
knockout_html = bracket_render.build_knockout_html(data)
champion_html = champion_render.build_champion_html(data)
